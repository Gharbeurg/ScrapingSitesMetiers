...
adresse_caducee_news = "https://www.caducee.net/actualite-medicale/"
adresse_caducee = "https://www.caducee.net"
fichier_caducee_sortie = "C:/DATA/github/.data/sante_news.csv"
...
f_log.write("{} - Lancement du programme - scraping_caducee_news.py\n".format(datetime.now().strftime("%d/%m/%Y %H:%M:%S")))
...
try:
    driver.get(adresse_caducee_news)
    ...
    result_block = soup_contenu.find_all("div", {"class":"post-desc"})
    ...
    if result_block:
        try:
            for result in result_block:
                ...
                #description, date
                if result.find("p", {"class":"post-chapo"}):
                    description_article = result.find("p", {"class":"post-chapo"}).text.strip()
                    limite = description_article.find(':')
                    date_article = description_article[:limite-1].strip()
                    date_article = date_article.replace("Caducee.net, le ", "")
                    description_article = description_article[limite+1:].strip()
                else:
                    description_article = ''
                    date_article = ''
                ...
                #lien, titre
                if result.find("a"):
                    lien_article = adresse_caducee + result.find("a").get('href')
...
df_news.to_csv(fichier_caducee_sortie, mode='a', sep=';', index=False, header=False)
...
#fin du programme
f_log.write("{} - Fin du programme - scraping_caducee_news.py\n".format(datetime.now().strftime("%d/%m/%Y %H:%M:%S")))
f_log.write("****************************************************\n")   